...
#         action.move_to_element(driver.find_element_by_id("dropdownCurrency")).click()
    ...
    def click_On_Section(self, category, item):
        ...
        wait.until(expected_conditions.presence_of_element_located((By.PARTIAL_LINK_TEXT, "Autothon Retail Demo Store - You Select We Deliver!!")))
        print(category +  item)
        print(category +  item)
        ...
        time.sleep(2)
        wait=WebDriverWait(driver, 20)
        wait.until(expected_conditions.presence_of_element_located((By.XPATH, "//button[contains(text(),'Add to Cart')]")))
